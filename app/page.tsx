...
export default function VibeSpyMain() {
  ...
  const handleAnalyze = async () => {
    ...
    } catch (e) { alert("Timeout. Apify is slow, check Archive in 2 minutes."); }
    setLoading(false);
  };
    ...
    <div className="min-h-screen bg-[#020617] text-slate-100 p-8">
      <div className="max-w-7xl mx-auto flex justify-between items-center mb-16">
        ...
        <div className="flex gap-2">
          <input value={input} onChange={e => setInput(e.target.value)} placeholder="Enter Page ID..." className="bg-slate-900 border border-slate-800 px-6 py-2 rounded-xl text-sm outline-none focus:ring-2 ring-blue-500" />
          <button onClick={handleAnalyze} disabled={loading} className="bg-blue-600 hover:bg-blue-500 px-10 py-2 rounded-xl font-bold uppercase text-xs active:scale-95 transition-all">
      ...
      <div className="max-w-7xl mx-auto space-y-20">
        {results.map((res, i) => (
          <div key={i} className="bg-slate-900/50 border border-slate-800 rounded-[3rem] p-12 shadow-2xl animate-in fade-in slide-in-from-bottom-4">
            <h2 className="text-5xl font-black mb-10 uppercase italic tracking-tight">{res.brand}</h2>
                  ...
                  <div key={ad.id} className="aspect-[9/16] bg-black rounded-2xl overflow-hidden border border-slate-800 relative group">
                    {ad.video ? (
                      ...
                      <div className="flex flex-col items-center justify-center h-full p-4 text-center">
                         ...
                         <span className="text-[10px] text-red-500 font-bold uppercase italic">Video Error</span>
                      </div>
                    )}
                  </div>
                ))}
